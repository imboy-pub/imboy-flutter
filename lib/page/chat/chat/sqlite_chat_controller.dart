...
class SqliteChatController
  ...
  Future<void> insertMessage(Message message, {int? index, bool animated = true}) async {
    ...
    _operationsController.add(ChatOperation.insert(message, insertIndex, animated: animated));
  }
  ...
  Future<void> insertAllMessages(List<Message> messages, {int? index, bool animated = true}) async {
    ...
    _operationsController.add(ChatOperation.insertAll(unique, insertIndex, animated: animated));
  }
  ...
  Future<void> removeMessage(Message message, {bool animated = true}) async {
    ...
    _operationsController.add(ChatOperation.remove(message, index, animated: animated));
  }
  ...
  Future<void> removeMessageById(String id) async {
    ...
    if (index == -1) {
      ...
      try {
        // 尝试从不同的消息表中查找消息
        final tables = ['message', 'group_message', 'c2s_message', 's2c_message'];
    ...
    try {
      ...
      iPrint('removeMessageById: UI列表移除消息完成: ${msg.id}, 剩余消息数量: ${_messages.length}');
    } catch (e) {
  ...
  Future<void> setMessages(List<Message> messages, {bool animated = true}) async {
    ...
    _operationsController.add(ChatOperation.set(List.unmodifiable(_messages), animated: useAnimation));
  }
  ...
  Future<void> scrollToBottom({bool animated = true}) async {
    try {
      await MessageScrollManager.to.scrollToBottom(animated: animated);
    } catch (_) {}
  }
