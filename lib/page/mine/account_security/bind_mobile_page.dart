...
class _BindMobilePageState extends ConsumerState<BindMobilePage> {
  ...
  void initState() {
    ...
    notifier.mobileCtl.addListener(() => notifier.updateMobile(notifier.mobileCtl.text.trim()));
    ...
    return IosPageTemplate(
      ...
      child: Column(
        children: [
          ...
          ImBoySettingsSection(
            children: [
              ImBoySettingsTile(
                ...
                subtitle: Text(hasBound ? hiddenPhone(currentMobile) : t.common.notBound),
                leading: Container(
                  width: 32, height: 32,
                  decoration: BoxDecoration(color: AppColors.getIosBlue(brightness), borderRadius: BorderRadius.circular(8)),
                  child: const Icon(CupertinoIcons.phone, color: Colors.white, size: 18),
                ),
                trailing: Container(
          ...
          // 输入表单 Section
          ImBoySettingsSection(
            header: Text((hasBound ? t.account.newMobile : t.account.mobile).toUpperCase()),
            children: [
              CupertinoListTile.notched(
                title: Text(t.account.mobile, style: const TextStyle(fontSize: 17)),
                trailing: Expanded(
                  child: PhoneInputWidget(
                    initialValue: '',
                    onInputChanged: (String full) => ref.read(bindMobileProvider.notifier).updateMobile(full),
          ...
          Padding(
            ...
            child: Column(
              children: [
                _ValidationRow(label: t.chat.formatCheck, ok: asyncState.mobileOk, text: asyncState.mobileOk ? t.main.correct : t.main.pendingInput),
          ...
          Padding(
            ...
            child: Text(t.common.verificationCodeSentToMobile, textAlign: TextAlign.center, style: const TextStyle(color: AppColors.iosGray, fontSize: 13)),
          ),
        ],
      ),
    );
  ...
  Widget _buildCodeButton(BuildContext context, WidgetRef ref, dynamic state) {
    return CupertinoButton(
      ...
      onPressed: state.canSendCode ? () async {
        final error = await ref.read(bindMobileProvider.notifier).sendCode();
      ...
      child: SizedBox(
        ...
        child: ElevatedButton(
          ...
          onPressed: state.canSubmit ? () async {
            final error = await ref.read(bindMobileProvider.notifier).submit();
