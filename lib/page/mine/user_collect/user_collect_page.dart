...
class _UserCollectPageState extends ConsumerState<UserCollectPage> {
  ...
  void _selectAllCurrent() {
    final currentState = ref.read(userCollectProvider);
    setState(() {
      for (final o in currentState.items) _selectedIds.add(o.kindId.toString());
      _multiSelect = true;
    });
  }
              ...
              children: [
                if (isPinned) ...[
                  Row(
                    mainAxisSize: MainAxisSize.min,
                    children: [
                      Icon(CupertinoIcons.pin_fill, size: 12, color: AppColors.getIosBlue(brightness)),
                      const SizedBox(width: 4),
                      Text(t.chat.pinned, style: TextStyle(color: AppColors.getIosBlue(brightness), fontSize: 11, fontWeight: FontWeight.w600)),
                    ],
                  ),
        ...
        child: Row(
          children: [
            if (_multiSelect) Padding(
              ...
              child: CupertinoCheckbox(value: isSelected, onChanged: (_) => _toggleSelect(obj), activeColor: AppColors.getIosBlue(brightness)),
            ),
            Expanded(child: card),
          ],
        ),
        startActionPane: ActionPane(
          ...
          children: [
            SlidableAction(
              ...
              icon: isPinned ? CupertinoIcons.pin_slash_fill : CupertinoIcons.pin_fill,
        ...
        content: Padding(
          ...
          child: Column(
            ...
            children: [
              Row(
                children: [
                  Avatar(imgUri: widget.peer['avatar'] ?? '', width: 44, height: 44),
                  const SizedBox(width: 12),
                  Expanded(child: Text(widget.peer['title'] ?? '', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w600))),
                ],
              ),
              const SizedBox(height: 12),
              Text(model.info['payload']?['text'] as String? ?? t.common.messageContent, maxLines: 3, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 14)),
            ],
          ),
        ),
        actions: [
      ...
      child: Row(
        children: [
          Padding(
            ...
            child: Text('${t.main.selected}: ${_selectedIds.length}', style: const TextStyle(fontWeight: FontWeight.w600)),
          ),
          const Spacer(),
          CupertinoButton(padding: const EdgeInsets.symmetric(horizontal: 8), child: const Icon(CupertinoIcons.square_list, size: 20), onPressed: _selectAllCurrent),
          CupertinoButton(padding: const EdgeInsets.symmetric(horizontal: 8), child: const Icon(CupertinoIcons.tag, size: 20), onPressed: _batchTag),
          CupertinoButton(padding: const EdgeInsets.symmetric(horizontal: 8), child: Icon(CupertinoIcons.delete, color: AppColors.getIosRed(brightness), size: 20), onPressed: _batchDelete),
          CupertinoButton(padding: const EdgeInsets.symmetric(horizontal: 8), child: const Icon(CupertinoIcons.xmark_circle, size: 20), onPressed: _exitMultiSelect),
        ],
      ),
    );
  }
      ...
      children: [
        Theme(
          ...
          child: ExpansionTile(
            ...
            title: Row(
              children: [
                Icon(CupertinoIcons.slider_horizontal_3, size: 18, color: AppColors.getIosBlue(brightness)),
                const SizedBox(width: 12),
              ...
              Padding(
                ...
                child: Wrap(
                  ...
                  children: kindMap.entries.map((e) {
                    ...
                    return ChoiceChip(
                      label: Text(e.value, style: TextStyle(fontSize: 13, color: isSelected ? Colors.white : null)),
                      selected: isSelected,
                      onSelected: (v) { if (v) _searchByKind(context, e.key, e.value); },
