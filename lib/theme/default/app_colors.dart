...
class AppColors {
  ...
  static const Color secondaryContainer = Color(0xFFD5E8D0);
  
  /// 次要色容器上的文本 - Material 3 On Secondary Container
  static const Color onSecondaryContainer = Color(0xFF101F0F);
  
  // ============ Material 3 第三色系统 ============
  /// 第三色 - Material 3 Tertiary
  /// 优化理由：微调蓝绿色调，增加视觉丰富度
  static const Color tertiary = Color(0xFF386667);
  
  /// 第三色容器 - Material 3 Tertiary Container
  static const Color tertiaryContainer = Color(0xFFBCEBF2);
  
  /// 第三色容器上的文本 - Material 3 On Tertiary Container
  static const Color onTertiaryContainer = Color(0xFF001F23);
  ...
  /// 暗色主题 - 背景颜色 - Material 3 Background (已废弃，使用Surface)
  ...
  // ============ 聊天相关颜色 ============
  /// 发送消息气泡背景 - 亮色主题
  /// 优化理由：使用主绿色确保白色文字的可读性，旧值 primaryGreenLight 过浅
  static const Color lightSentMessageBackground = primaryGreen;
  
  /// 接收消息气泡背景 - 亮色主题
  static const Color lightReceivedMessageBackground = Color(0xFFFFFFFF);
  
  /// 发送消息气泡背景 - 暗色主题
  /// 优化理由：深色模式下使用醒目但舒适的绿色，保持视觉识别度
  ...
  /// 消息已读状态
  static const Color messageRead = primaryGreen;
  ...
  /// 主绿色 - 20% 透明度
  static Color get primaryGreenAlpha20 => primaryGreen.withValues(alpha: 0.2);
  
  /// 主绿色 - 30% 透明度
  static Color get primaryGreenAlpha30 => primaryGreen.withValues(alpha: 0.3);
  
  /// 主绿色 - 50% 透明度
  static Color get primaryGreenAlpha50 => primaryGreen.withValues(alpha: 0.5);
  ...
  /// [brightness] 主题亮度
  /// [isSecondary] 是否为次要文本
  static Color getEyeCareTextColor(bool isEyeCareMode, Brightness brightness, {bool isSecondary = false}) {
    if (brightness == Brightness.light) {
      return isSecondary ? lightTextSecondary : lightTextPrimary;
    }
    if (isEyeCareMode) {
  ...
  /// [isOLEDMode] 是否启用OLED模式
  /// [brightness] 主题亮度
  static Color getChatBubbleBackground(bool isSent, bool isOLEDMode, Brightness brightness) {
    if (brightness == Brightness.light) {
      return isSent ? lightSentMessageBackground : lightReceivedMessageBackground;
    }
    if (isSent) {
      return darkSentMessageBackground;
    }
    return isOLEDMode ? oledReceivedMessageBackground : darkReceivedMessageBackground;
  }
  ...
  static bool checkContrastRatio(Color foreground, Color background, {String level = 'AA'}) {
