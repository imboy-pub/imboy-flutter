...
/// 消息滚动管理器
/// 处理消息列表的滚动、定位和动画效果
class MessageScrollManager extends GetxController {
  static MessageScrollManager get to => Get.find();
  ...
  void _initScrollListener() {
    scrollController.addListener(() {
      ...
      isAtBottom.value = maxScroll - currentScroll <= _scrollThreshold;
      
      // 检测滚动状态
      if (isScrolling.value && !scrollController.position.hasPixels) {
  ...
  void cacheMessagePosition(
    ...
    if (conversationCache.length > 500) { // 最多缓存500个消息位置
  ...
  void handleNewMessage(String conversationId, Message message) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (scrollController.hasClients) {
        ...
        // 更新附近消息的位置缓存
        _updateNearbyMessagePositions(conversationId, message.id, estimatedPosition);
      }
    });
  }
...
class ScrollToBottomButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return GetBuilder<MessageScrollManager>(
      ...
      builder: (controller) {
        // 如果在底部或没有新消息，不显示按钮
        if (controller.isAtBottom.value && unreadCount == 0) {
        ...
        return Positioned(
          ...
          child: AnimatedOpacity(
            opacity: controller.isScrolling.value ? 0.5 : 1.0,
...
/// 消息位置监听器组件
class MessagePositionListener extends StatelessWidget {
  ...
  /// 更新消息位置
  void _updateMessagePositions() {
    final scrollManager = MessageScrollManager.to;
    if (!scrollManager.scrollController.hasClients) return;
    ...
    scrollManager.cacheMessagePositions(
      conversationId,
      positions,
    );
  }
