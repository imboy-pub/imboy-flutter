...
// 上限：只认顶层「key: value」一行一项——嵌套 / 列表 / 多行字符串 / 锚点全不
// 支持，写成嵌套就静默解析不到（好在 _require + _stamp 的 fail-fast 会拦下，
// 不会打出半成品的标）；行内注释也只按「空格+#」粗切。
// 升级触发：brand.yaml 需要嵌套结构时（典型是按 flavor 分多套品牌、或成套的
// 颜色 token），换 package:yaml；替换文本本身开始需要条件/循环时，才谈模板引擎。
