...
public class SwiftAudioWaveformsPlugin: NSObject, FlutterPlugin {
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        // imboy 本地修复（2026-08-10）：
        // 上游 2.0.2 在 iOS 16.x 上注册即 SIGSEGV（swift_getObjectType at 0x0），
        // pub.dev 无修复版本。vendored 后置空注册：插件方法调用返回
        // MissingPluginException，由 Dart 侧兜底（波形 UI 降级），
        // app 启动不再崩溃。语音录制/播放不受影响（flutter_sound/just_audio）。
        _ = registrar
    }
