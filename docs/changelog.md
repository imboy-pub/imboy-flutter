...
# 1.0.0-alpha.15
* 端到端加密升级：单聊改用 Olm 协议（X3DH + Double Ratchet），群聊房间密钥经 Olm 分发
* 修复联系人列表悬浮字母索引的滚动卡顿
* 修复个人背景图上传失败
* 「我的」下属页面多处崩溃与空数据防护

# 1.0.0-alpha.14
* 频道全面改版：沉浸式全屏阅读、图文撰写（标题 + 封面 + 预览）、Markdown 正文、格式工具条
* 频道内容流由聊天气泡改为订阅号式内容卡
* 朋友圈：九宫格标准化、单视频宽高比、内联评论预览、评论数外显、支持所在位置与 @提醒
* 消息输入框四项精修（输入态、聚焦、字数、发送态）

# 1.0.0-alpha.13
* AI 助手：流式回复逐字显示，群内任务可观测与审批卡片，任务实时进度
* 朋友圈视觉重构：图片占位、空状态动效、点赞动效
* 群协作页统一 Cupertino 风格与卡片视觉
* 新增举报入口与聊天背景设置入口

# 1.0.0-alpha.12
* 拍摄统一改用微信风格相机
* 位置选点页按钮死态、触达区与无障碍问题修复；关闭逆地理请求，定位更快
* 修复端到端加密本地备份的头部长度错位与密钥分块加解密不对称
* 修复数据库建库/迁移在部分设备上的加载失败风险

# 1.0.0-alpha.11
* 面对面建群界面重做
* 全应用图片支持点击放大、双指缩放，支持 HEIC 解码
* 聊天语音支持微信式上滑取消与转文字，新增文本预览编辑
* 新增暗色模式边框修复与动画规范
* 补齐密码显隐、闪光灯、扫描暂停、全屏、语音键盘切换的 10 语言翻译

# 1.0.0-alpha.9
* 音视频通话界面重新设计
* 付费频道：订单历史、订单详情与退款入口
* 钱包提现入口接通
* WebSocket 消息重试策略优化，新增发送中间态

# 1.0.0-alpha.8
* 新增表情包选择器，改进表情渲染
* 新增连接状态指示
* 附件访问控制：头像公开读、聊天附件按会话授权

# 1.0.0-alpha.1
* 1.0 系列起点，架构与基础能力大幅重构

# 0.7.0
* 重构主题系统：实现完整的Material 3主题管理，支持动态字体缩放、OLED优化和护眼模式
* 优化消息架构：重构聊天模块采用Mixin模式，新增消息状态管理、性能监控和音视频处理优化
* 改进用户界面：重新设计个人资料页面，完善深色模式适配，优化聊天交互体验
* 技术债务清理：移除niku依赖，升级核心依赖包，优化数据库结构和网络监控
* 应用图标更新：统一Android、iOS、macos平台应用图标设计
* 版本号调整：0.6.1 → 0.7.0

# 0.6.1
* RSAService 优化；其他一些语法调整；
* 优化群头像UI显示效果；
* 升级gradle到8.12，并调整相关依赖
* 新增修改Android包名的脚本；优化等于页面兼容性问题；DottedBorder 升级接口；
* 会话管理优化；
...
# 0.5.11
* 新增保存聊天中的图片、语音文件、视频文件、附件文件等到磁盘的功能

# 0.5.10
* 调试 webrtc 连通性（效果不明显，有待继续测试）；
* 修复账号登录只能使用Email的问题（现在可以使用系统分配的账号登录了）；
* 修复本地时间显示有误的问题；
* 升级一些依赖(flutter升级到 Flutter 3.24.3 • channel stable)；

# 0.5.9
* 修复 r_upgrade 插件，解决 Android10 没法下载升级包的问题；
