...
- ✅ 点赞 / 更多操作 sheet（赞·评论·取消）/ 消息通知中心（空状态）/ 动态详情页（评论输入+发送）均正常。
- ⚠️ 首条自发 test 动态图 broken_image：该条图片 URL 已失效（errorBuilder 正常，非 bug）。
- 0 新 bug，0 需修复。

### 🔴 P0 — 朋友圈拍照上传失败（2026-07-11/12，⚠️后端已全排除，真因在客户端）
- **现象**：拍照 → 预览确认后 toast「媒体上传失败」或静默无反馈，媒体网格无缩略图。
- **⚠️ 首轮误判纠正**：曾据本地 `sys.config`/`deploy/.env.example` 模板**推测**根因=后端 `IMBOY_GARAGE_ENDPOINT=http://garage:3900`（docker 内网），并提交 commit 15313b22。**此结论错误**。教训：勿用本地配置模板推断生产实际配置，必须登生产核实。
- **后端实测全部正常（已 SSH 106.53.76.53 核实，用户授权）**：
  - 生产非 docker（裸机 release 蓝绿），garage systemd，openresty(宝塔)已按 X-Amz 签名分流→3900(Host 保留)。
  - 当前 release `sys.config` garage `endpoint` **早已=`https://s3.imboy.pub`** + `internal_endpoint` 已分离。
  - `bin/imboy eval presign` 生成的 put_url host=公网、AWS4 签名齐全。
  - **本地 curl + dart-dio（复制 `_rawDioPut` 方式）PUT 该真实 URL，6 字节与 2MB 文件均 HTTP 200 落桶**。→ 后端/nginx/garage/presign/TLS 全链路正常，**用户无需改生产任何配置**。
- **真因（客户端）**：`attachment_api._rawDioPut` 用 `Stream<List<int>>.fromIterable([bytes])` body，Android dart:io HttpClient 对 Stream body 发 **chunked transfer-encoding**（忽略手动 content-length），而 Garage/S3 presigned PUT 要求 `Content-Length` → 真机 PUT hang（mac 正常）。被 `uploadFileViaPresignCompat` catch 静默吞异常掩盖，logcat 无痕。
...
### 轮 3（2026-07-12）我的/设置 mine — 0 bug
- mine 主页 ✅（用户信息/二维码/钱包/我的频道/收藏/存储空间/登录设备管理/设置/反馈建议）、设置页 ✅（账号安全/语言/深色模式/字体大小/允许搜索我开关/刷新设备密钥/E2EE密钥管理/更新日志/帮助文档/隐私政策/关于应用/退出登录/注销账号）。~22 功能点全通过，0 bug。
- 未深入：账号安全子页(邮箱/手机敏感,只读核验入口)、收藏/存储/设备子页，未实改任何设置。
...
### 轮 7（2026-07-12）个人资料 personal_info — 0 bug
- 个人信息页 ✅（头像/编辑头像/资料完善度/昵称/性别/邮箱/手机/个性签名/个人背景/职业/学校/兴趣爱好/二维码/隐私设置入口）、隐私设置页 ✅（5 开关：账号搜索/手机号添加/二维码添加/显示在线状态/附近的人可见）。~20 功能点全通过，0 bug。
- 未深入：各字段编辑弹窗+校验(入口存在,邮箱敏感未实改)、未实改任何隐私开关。

### 轮 8（2026-07-12）用户标签 user_tag — 0 bug
- 标签列表页 ✅（新建/搜索/标签列表"感觉"）、新建标签页 ✅（标签名输入框/完成）。~6 功能点通过，0 bug。
- 未深入：标签详情/选好友添加成员/关系 TagInput 校验（入口存在，成本限制）。

### 轮 9（2026-07-12）钱包 wallet — 0 bug
- 钱包主页 ✅（总资产/收付款/零钱/银行卡/腾讯服务九宫格/流水记录）~12 功能点通过，0 bug。
- 未深入：零钱/收付款/流水/银行卡子页（入口存在，成本 $472 限制；阶段0/1 wallet API 已覆盖）。

### 轮 10（2026-07-12）二维码/扫码/搜索 — 0 bug
- 我的二维码页 ✅（头像/地区/二维码/保存/分享）、扫一扫入口 ✅、搜索入口 ✅（联系人/消息/频道/群，前轮见）。~8 功能点通过，0 bug。
- 未深入：扫一扫深度(闪光灯/相册/分流/扫码登录)、搜索深度(防抖/历史/定位)需实操。

### 轮 11（2026-07-12）聊天设置/通话 — 0 bug
- 单聊聊天设置页 ✅（标准模式/消息免打扰/阅后即焚/查找聊天记录/聊天背景/清空聊天记录）、通话入口 ✅（联系人资料页语音/视频通话，不实拨）。~8 功能点通过，0 bug。
- 未深入：chat 主体(发消息/长按菜单/画廊/@引用/转发/快捷回复)、实际通话。

### 轮 12（2026-07-12）E2EE 页（末模块）— 0 bug
- E2EE 密钥管理页 ✅（密钥信息:已启用/已激活/设备ID/密钥ID/创建时间、关于 E2EE 说明、三恢复方法:设备间传输/社交恢复/本地备份）。~12 功能点通过，0 bug。kid 与 device_id 正确分离。
- 未深入：3 恢复方法子页深度(二维码传输/社交恢复/本地备份导出)需实操。

### 🎉 真机 QA loop 全部 11 模块推进完毕（2026-07-12）
- ✅ 0-bug 通过：moment · contact · mine · channel · personal_info · user_tag · wallet · qrcode/scanner/search · chat设置/通话 · E2EE（10 模块）
- 🔧 已修待真机复验：group 详情页加载卡死(7c1dd353)；另 moment 拍照上传(f9d4af7e)
- 共发现并修复 3 个 bug（moment 红框 eb71ea2c 已复验 + 拍照上传 f9d4af7e + 群详情卡死 7c1dd353）
- 遗留：各模块深度子页（编辑弹窗/实操流程/付费墙/通话）因成本+敏感未深入；两个已修 bug 待用 arm64/release 小包真机复验。
