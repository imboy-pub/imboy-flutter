...
void main() {
  group('UpgradeTimerPolicy.computeInterval', () {
    test('intervalHours=0 返回 null（不启动定时器，防死循环）/ '
        'returns null to disable timer (prevents tight loop)', () {
      expect(UpgradeTimerPolicy.computeInterval(0), isNull);
    });

    test('intervalHours<0 返回 null / negative returns null', () {
      expect(UpgradeTimerPolicy.computeInterval(-1), isNull);
      expect(UpgradeTimerPolicy.computeInterval(-24), isNull);
    });

    test('intervalHours=1 返回 1 小时 / 1 returns Duration(hours: 1)', () {
      expect(UpgradeTimerPolicy.computeInterval(1), const Duration(hours: 1));
    });

    test('intervalHours=24（默认）返回 24 小时 / default 24 returns 24 h', () {
      expect(UpgradeTimerPolicy.computeInterval(24), const Duration(hours: 24));
    });

    test('intervalHours=168（1 周）返回 168 小时 / 168 returns one week', () {
      expect(
  ...
  group('VersionComparator 边界 / edge cases', () {
    test('"1.0" 两段式补齐为 1.0.0 / two-segment pads to 1.0.0', () {
    ...
    test('超长段数字仍可比较 / large segment numbers compare correctly', () {
      expect(VersionComparator.compare('999999.0.0', '999998.9.9'), 1);
      expect(VersionComparator.compare('1.0.9999', '1.0.10000'), -1);
    });

    test('四段式只比较前三段 / four-segment compares only first three', () {
  ...
  group('AppVersionInfo.fromJson 容错 / fault tolerance', () {
    ...
    test('null 字段不崩溃 / null fields do not crash', () {
      final info = AppVersionInfo.fromJson(<String, dynamic>{
        'vsn': null,
        'download_url': null,
        'description': null,
    ...
    test('changelog 是字符串时回退为空列表 / string changelog falls back to empty', () {
    ...
    test('changelog List 内含非 Map 元素被过滤 / non-Map items filtered out', () {
      final info = AppVersionInfo.fromJson(<String, dynamic>{
        'vsn': '1.0.0',
        'changelog': [
          {'tag': 'feat', 'text': 'ok'},
    ...
    test('hasUpdate 要求 updatable=true 且 upgradeType!="none" / '
    ...
    test('fileSize 边界 1023 / 1024 / 1024*1024 / boundary sizes', () {
      expect(
