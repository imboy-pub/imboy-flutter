...
现有 `integration_test/e2ee_olm_device_test.dart`、`integration_test/mine/mine_subpages_smoke_test.dart` 及备份 widget 测试已覆盖当前安全可执行范围；暂不新增包装测试。双设备和恢复测试必须使用非生产账号、受控密钥材料及显式授权。
