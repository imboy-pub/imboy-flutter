...
- 2026-08-18（DEMO-FLOW-20260818）：复跑 `live_room_list_provider_test.dart`（`flutter test --concurrency=1`）
  `12` 项全部通过（列表加载、分页 loadMore、加载态防重入），与 08-17 一致。
- 2026-08-18（DEMO-FLOW-20260818）：本地登录后 API 只读回读复验（测试账号 13900001002，纯 GET 无写入）：
  `list` → `code=0` 空列表（`total=0` 正常空态）；`my_list` → `code=0` 空列表；
  `detail?room_id=999999999999` → `code=1 直播间不存在`（无效房间边界正确）。三项与 08-17 结果一致。
  开播/观看阻塞维持（livekit 配置缺失与 DF-21 同根因，见上条）。
- 2026-08-19（DEMO-FLOW-20260819）：复跑 `live_room_list_provider_test.dart`（`flutter test --concurrency=1`）
  `12` 项全部通过（列表加载、分页 loadMore、加载态防重入），与 08-17/08-18 一致；本轮未出现
  Flutter 3.47.0 text_painter 编译错误。
- 2026-08-19（DEMO-FLOW-20260819）：本地登录后 API 只读回读复验（测试账号 13900001002，纯 GET 无写入，
  一次性探针复用 `ApiTestClient` 签名，仅本地地址门禁，用后已删）：`list` → `code=0 total=0`（正常空态）；
  `my_list` → `code=0` 空列表；`detail?room_id=999999999999` → `code=1 直播间不存在`（无效房间边界正确）。
