...
现有 `group/group_collaboration_readonly_test.dart` 保留为低风险只读回归；`integration_test/demo_flow/group_collaboration_flow_test.dart` 负责显式授权后的双账号闭环。测试使用既有 P0 测试群，不执行取消日程、删除任务、撤销投票或费用结算。
