...
> 状态：`本地 API 级全部闭环并复跑通过（2026-08-19 alpha.36 复跑维持：建群/面对面含 face2face_save 落库加严断言 3/3、群管理含群主转让+限流负向 4/4、消息密文归档 2/2、协作 4/4+生产只读 12 过 4 门禁拦；P1 群内容上传授权闭环历史首次通过、群组织二维码恢复 4/4）/ 面对面 UI 双端未覆盖 / msg_page 列名 bug 后端仍未修`
> 优先级：P0
> 类型：群功能流程索引
...
- 2026-08-19：8 并行会话轮次复跑维持，各专题详见自身文档：DF-07 `3/3`（face2face 加严断言连续第三天全绿，
  新建 f2f 群 `107851069891282944`）、DF-09 `4/4`（群主转让在新专用群 `107851155283118080` 复现，含
  group_log type=9 与 per_hour_once 限流负向）、DF-08 `2/2`（msg_page 键名 bug 三重证据仍未修维持）、
  DF-10 `4/4` + 生产只读 bake key 补跑 `12 过 4 门禁拦`（与 08-17/18 口径一致）。P1 两个升级项：
  DF-14 群内容 `3/3`（Garage endpoint 已由人工修复为 127.0.0.1:3900，文件/照片上传+view_url 授权下载
  200 内容一致历史首次闭环）；DF-15 群组织 `4/4`（群二维码 tk 校验恢复 code=0，302 回归解除——
  根因确认为注入带引号 SOLIDIFIED_KEY 误判，见 qrcode_invite_flow.md 08-19 裁决记录）。
