...
MigrationScript syntheticUpgradeV11() => MigrationScript(
  ...
  targetVersion: 11,
  description: 'v10 → v11',
  sqlStatements: ['ALTER TABLE sessions ADD COLUMN expires_at INTEGER;'],
);
...
void main() {
  ...
  group('多级升级 V9→V11 / multi-version upgrade chain', () {
    ...
    setUp(() async {
      ...
      scripts = {10: syntheticUpgradeV10(), 11: syntheticUpgradeV11()};
    });
    ...
    test('planner 给出升序 [V10, V11] 的执行顺序 / plan is ascending', () {
    ...
    test('按升序执行能成功升级 V9→V11 / ascending plan completes successfully', () async {
      // 插入 V9 样本数据
      ...
      expect(sessCols, containsAll({'id', 'user_id', 'token', 'expires_at'}));
    ...
    test(
      ...
      () async {
        ...
        await expectLater(
          ...
          reason:
              '证明：降序执行在 V9 schema 下必然失败（V11 依赖 V10 的 sessions 表），'
              '升序执行才正确',
        );
      },
    );

    test('单级升级 V9→V10 不受影响 / single-hop upgrade unaffected', () async {
      final plan = MigrationScriptPlanner.plan(
        scripts: scripts,
        fromVersion: 9,
        toVersion: 10,
    ...
    test('从中间版本继续升级 V10→V11 只执行 V11 / intermediate upgrade V10→V11', () async {
      // 先把 DB 升到 V10
