...
|---|---|---|---|---|---|---|---|---|---|
| 回归复测 | 2026-08-08 | `page/chat/p2p_call_screen/p2p_call_screen_page.dart` | 发起音视频呼叫并邀请对端 | 待重验 | 批次71 | 0 | 0 | 0 | 真机对 leeyi(50075) 发起语音呼叫：呼叫页打开「等待对方接受邀请...」文案正确；⚠️偶现 FlutterWebRTC NPE（SurfaceTextureSurfaceProducer.release null，语音 renderer 未渲染即 dispose，首次触发第二次未复现）已记 BUG#145；接听闭环仍需对端接听 |
...
| 回归复测 | 2026-08-08 | `page/chat/p2p_call_screen/p2p_call_screen_page.dart` | 切换扬声器与前后镜头 | 待重验 | 批次71 | 0 | 0 | 0 | 真机通话页扩音器开关 checked→未勾选（扬声器切换生效）；前后镜头切换需视频通话+对端另验 |
