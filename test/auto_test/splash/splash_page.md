...
| 无待办 | - | `page/splash/splash_page.dart` | 宽屏已登录跳 Web Shell 三栏壳 | 已通过 | 批次104 | 0 | 0 | 0 | 真机（0817 批次104，MRD-AL00）：`wm density 90` 造宽屏（720 物理 / 0.5625 = 逻辑宽 1280 ≥ ultraWide 1200）→ 冷启动 uiautomator 原始 XML 铁证三栏分界（左导航 消息/联系人/频道/我的 x=[0,41] + 会话列表 x=[41,243] + 第三栏「IMBoy」@x=462）；源码 splash_page L155-157 resolveShellLayout(width)→threeColumn→context.go('/web_shell') 成立；`wm density reset` 后冷启动回底部导航四 tab（y=1399），mobile 分支对照闭环 |
