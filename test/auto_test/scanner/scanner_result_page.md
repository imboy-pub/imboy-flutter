...
| 待修复 | 2026-08-13 | `page/scanner/scanner_result_page.dart` | 非链接结果时浏览器按钮置灰 | 有BUG待修 | 批次75 | 1 | 0 | 1 | BUG#批次75-1：扫描纯文本 qr_text.png→结果页浏览器按钮 enabled=true/clickable=true **未置灰**，点击无反应（视觉误导用户以为可点）。期望非 URL 结果按钮置灰或隐藏 |
...
| 阻塞 | 第二台设备扫码可达后 | `page/scanner/scanner_result_page.dart` | 超长结果文本不溢出屏幕 | 未测 | - | 0 | 0 | 0 | 待用 qr_long.png 验证（二维码已生成在 /tmp，待后续批次） |
