...
| 阻塞 | 待新APK | `page/user_tag/user_tag_relation/tag_relation_page.dart` | 点重置恢复到进页原始标签 | BUG已修待验 | 批次72 | 1 | 1 | 0 | 真机复现（BUG#131 修复后入口可达）：重置按钮 enabled/disabled 随变更切换正常，但点重置后**已选标签区块不恢复**（仍显示 qa_test2 (2/20)）→ BUG#142：页面 _currentTags 与 TagInput 内部双状态脱节（initialTags 只在 initState 读取）。已修（c53bc3cd 08-08 15:38）：_tagInputVersion 计数器 + ValueKey 强制重建，清空/重置时自增；2 回归测试全绿 + user_tag 全目录 40 绿 + analyze 零 issues。批次36 §3.2 判定：c53bc3cd 晚于 APK（14:55:39）不在当前 APK 内，转阻塞等新 APK 复验 |
| 阻塞 | 待新APK | `page/user_tag/user_tag_relation/tag_relation_page.dart` | 点清空弹出二次确认后清空 | BUG已修待验 | 批次72 | 1 | 1 | 0 | 真机：二次确认弹窗正常（确认清空/确定要清空所有标签吗？/取消/确定，tag_relation_page.dart L166-189）；确认后统计卡已选→0、保存入口出现、清空禁用（页面态已清）但**已选标签区块残留 qa0804 (1/20)**、点建议 qa0804 无反应（TagInput 内部 contains 去重拦截）→ 同 BUG#142，已修（c53bc3cd 08-08 15:38，见重置行备注）。批次34 §3.2 判定：c53bc3cd 晚于 APK（14:55:39）不在当前 APK 内，转阻塞等新 APK 复验 |
