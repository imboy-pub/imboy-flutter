...
| 待复验 | 2026-08-13 | `page/channel/channel_edit_page.dart` | 自定义 ID 为空可补设一次、已设则锁定 | 待重验 | 批次18 | 1 | 1 | 0 | 本轮决定性验证：初版后端修复（resolve_custom_id_update/2）含 Erlang 同名变量陷阱——内层 case 模式变量 Channel 与外层已绑定 Channel 同名→编译为相等匹配→三分支永不命中→case_clause；已改内层变量为 Found，指令级验证 map_size 作用于 subject，eunit channel_logic_message 12+channel_logic 196 全绿；剩余=空闲真机补设复验 |
...
| 待复验 | 2026-08-13 | `page/channel/channel_edit_page.dart` | 保存失败错误提示与按钮恢复 | 待重验 | 批次67 | 2 | 2 | 0 | BUG#134 后端修复本轮决定性验证：初版 resolve_custom_id_update/2 含 Erlang 同名变量陷阱（内层 case 变量 Channel 与外层已绑定同名→相等匹配→case_clause），已改内层变量为 Found（src/logic/channel_logic_message.erl），指令级验证+eunit 12+196 全绿；BUG#136 客户端 catch toast 修复已随 APK（08-09 14:51 装机）；剩余=生产 PUT 响应需授权+空闲真机复验 |
