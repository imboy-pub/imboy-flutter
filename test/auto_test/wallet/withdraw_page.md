...
| 阻塞 | 需余额>0 且用户授权 | `page/wallet/withdraw_page.dart` | 二次确认展示金额渠道账号 | 待重验 | - | 0 | 0 | 0 | 批次33 复核：_handleWithdraw 中 amountYuan>maxBalanceYuan 直接 showError return（余额 ¥0.00 实测），二次确认弹窗不可达，非回归系缺外部条件转阻塞；0816复核:余额已足¥28.32(生产DB实测),仍缺资金测试授权 |
...
| 阻塞 | 需余额>0 且用户授权 | `page/wallet/withdraw_page.dart` | 提交提现请求并刷新余额返回 | 未测 | - | 0 | 0 | 0 | 0816复核:余额已足¥28.32(生产DB实测),仍缺资金测试授权 |
...
| 阻塞 | 需余额>0 且用户授权 | `page/wallet/withdraw_page.dart` | 提现失败错误提示 | 未测 | - | 0 | 0 | 0 | 0816复核:余额已足¥28.32(生产DB实测),仍缺资金测试授权 |
