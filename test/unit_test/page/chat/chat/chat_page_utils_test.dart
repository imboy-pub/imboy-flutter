...
void main() {
  ...
  CustomMessage buildCustom({
    ...
    return CustomMessage(authorId: authorId, id: 'c1', metadata: metadata);
  }
  ...
  group('canEditMessage', () {
    ...
    test('当前用户 + 文本 + 14 分钟前 → true（边界内）', () {
      final edge = DateTime.now().toUtc().subtract(const Duration(minutes: 14));
      final msg = buildText(createdAt: edge);
      expect(ChatPageUtils.canEditMessage(msg, uidMe), isTrue);
    });

    test('当前用户 + 文本 + 16 分钟前 → false（窗口外）', () {
      final over = DateTime.now().toUtc().subtract(const Duration(minutes: 16));
  ...
  group('getBurnAfterMs', () {
    ...
    test("burn_after_ms 优先于 expiry_time", () {
      final msg = buildText(
        metadata: const {'burn_after_ms': 10000, 'expiry_time': 99999},
      );
  ...
  group('withBurnMetadata', () {
    test('burnEnabled=false → 原 base 不变（引用相同）', () {
      ...
      expect(identical(result, base), isTrue, reason: '不启用时直接返回原 Map，避免无谓复制');
    });

    test(
      'burnEnabled=true → 新 Map + burn=true + burn_after_ms + expire_secs',
      () {
        final base = <String, dynamic>{'msg_type': 'text'};
        final result = ChatPageUtils.withBurnMetadata(
          base: base,
  ...
  group(
    'isVideoMessage / isAudioMessage / isFileMessage (metadata.msg_type)',
    () {
      test("metadata['msg_type']='video' → isVideo=true", () {
        final msg = buildCustom(
          metadata: const {'msg_type': MessageType.video},
        );
      ...
      test("metadata['msg_type']='voice' → isAudio=true", () {
        final msg = buildCustom(
          metadata: const {'msg_type': MessageType.voice},
        );
  ...
  group('isFromCurrentUser', () {
    ...
    test('authorId != uid → false', () {
      expect(
        ChatPageUtils.isFromCurrentUser(buildText(authorId: uidOther), uidMe),
        isFalse,
      );
    });
