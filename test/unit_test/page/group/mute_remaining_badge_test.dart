...
void main() {
  ...
  group('MuteRemainingBadge — 可见性', () {
    testWidgets('muteUntilMs == null → 不渲染任何文本', (tester) async {
      await tester.pumpWidget(
        wrap(const MuteRemainingBadge(muteUntilMs: null, nowMs: nowMs)),
      );
    ...
    testWidgets('已过期（muteUntilMs <= nowMs）→ 不渲染', (tester) async {
      await tester.pumpWidget(
        wrap(const MuteRemainingBadge(muteUntilMs: nowMs - 1, nowMs: nowMs)),
      );
    ...
    testWidgets('剩余 5 分钟 → 显示 "5 分钟"（含"禁言"前缀）', (tester) async {
      await tester.pumpWidget(
        wrap(
          MuteRemainingBadge(muteUntilMs: nowMs + 5 * 60 * 1000, nowMs: nowMs),
        ),
      );
  ...
  group('MuteRemainingBadge — 语义样式', () {
    testWidgets('使用破坏性语义色（error / red 系）', (tester) async {
      await tester.pumpWidget(
        wrap(MuteRemainingBadge(muteUntilMs: nowMs + 30 * 1000, nowMs: nowMs)),
      );
