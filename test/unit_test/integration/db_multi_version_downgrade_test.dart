...
void main() {
  ...
  group('多级降级 V11→V9 / multi-version downgrade chain', () {
    ...
    setUp(() async {
      ...
      scripts = {10: syntheticDowngradeV10(), 11: syntheticDowngradeV11()};
    });
    ...
    test('planner 给出降序 [V11, V10] 的执行顺序 / plan is descending', () {
    ...
    test('按升序执行（模拟 bug）会失败 / ascending plan fails (regression guard)', () async {
    ...
    test('单级降级 V10→V9 不受影响 / single-hop downgrade unaffected', () async {
      // 另起一个 V10 的 DB
      final v10Db = await databaseFactory.openDatabase(inMemoryDatabasePath);
      try {
        await v10Db.execute('''
